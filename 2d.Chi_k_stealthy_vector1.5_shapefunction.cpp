...
//modified: 08/11/2022 by Yang Jiao, yang.jiao.2@asu.edu
...
//version1.5: the shape function of a pixel is turned on in this version
//initial studies indicates the final results strongly depend on initial configurations
...
void get_region_circle_ring()
{
	...
	for(int i = 1; i <= Nk; i++)
		for(int j = - Nk; j <= Nk; j++)
		{
			...
			if(temp_dist<=R_c && temp_dist>=R_in)
				{
					vector_region[ct_region][0] = i;
					vector_region[ct_region][1] = j; //this can be negative, but converted to positie by a translation
...
void get_region_ellipse()
{
	cout<<"computing k vectors for the constrained ELLIPSE region..."<<endl;
	cout<<"K_bound = "<<K_bound<<endl;
	
	double R_c; //long axis
	cout<<"R_circle ="; cin>>R_c;
	if(R_c>=K_bound)
	{
		cout<<"R_c > K_bound! Abort!"<<endl;
	...
	double gamma;
	cout<<"aspect ratio gamma (>1) = "; cin>>gamma;
	double R_b = R_c/gamma; //short axis
	...
	for(int i = 1; i <= Nk; i++)
		for(int j = - Nk; j <= Nk; j++)
		{
			double temp_dist = sqrt((i/R_c)*(i/R_c)+(j/R_b)*(j/R_b)); //distance to origin
...
void get_region_square()
{
	cout<<"computing k vectors for the constrained SQUARE region..."<<endl;
	cout<<"K_bound = "<<K_bound<<endl;
	...
	for(int i = 1; i <= Nk; i++)
		for(int j = - Nk; j <= Nk; j++)
		{
			//double temp_dist = sqrt(pow(i/R_c, 4)+pow(j/R_b, 4); //distance to origin
			
			if(abs(i)<=R_c && abs(j)<=R_b)
				{
					vector_region[ct_region][0] = i;
					vector_region[ct_region][1] = j; //this can be negative, but converted to positie by a translation
...
void get_region_diamond()
{
	cout<<"computing k vectors for the constrained DIAMOND region..."<<endl;
	cout<<"K_bound = "<<K_bound<<endl;
}

void get_region_rectangle()
{
	cout<<"computing k vectors for the constrained RECTANGLE region..."<<endl;
	cout<<"K_bound = "<<K_bound<<endl;
}

void get_region_superdisk_square()
{
	cout<<"computing k vectors for the constrained SQUARE SUPERDISK region..."<<endl;
	cout<<"K_bound = "<<K_bound<<endl;
...
void get_region_superdisk_diamond()
{
	cout<<"computing k vectors for the constrained DIAMOND SUPERDISK region..."<<endl;
	cout<<"K_bound = "<<K_bound<<endl;
	...
	for(int i = 1; i <= Nk; i++)
		for(int j = - Nk; j <= Nk; j++)
		{
			double temp_dist = sqrt(pow(i/R_c, 1.5)+pow(j/R_b, 1.5)); //distance to origin
...
void get_region_superdisk_concave()
{
	cout<<"computing k vectors for the constrained CONCAVE SUPERDISK region..."<<endl;
	cout<<"K_bound = "<<K_bound<<endl;
	...
	for(int i = 1; i <= Nk; i++)
		for(int j = - Nk; j <= Nk; j++)
		{
			double temp_dist = sqrt(pow(fabs(i/R_c), 0.5)+pow(fabs(j/R_b), 0.5)); //distance to origin
...
void get_region_remiscate()
{
	cout<<"computing k vectors for the constrained REMISCATE region..."<<endl;
	cout<<"K_bound = "<<K_bound<<endl;
}
	...
	for(int i = -Nk; i <0 ; i++)
		  ...
		  for(int j=-Nk; j<=Nk; j++)
		  	{	
				...
				Chi_k_vectorb[i+Nk][j+Nk] = Chi_k_vectorb[Nk-i][Nk-j];
				Chi_k_vector[i+Nk][j+Nk] = Chi_k_vector[Nk-i][Nk-j];
		  	}		
    	}
    	
    for(int j=-Nk; j<=0; j++)
    	{
    		Chi_k_vectorb[0][j+Nk] = Chi_k_vectorb[0][Nk-j];
			Chi_k_vector[0][j+Nk] = Chi_k_vector[0][Nk-j];
		}
}
...
int main()
{
	...
	get_region_circle();
	//get_region_circle_ring();
	//exit(1);
	//get_region_ellipse();
	//get_region_square();
	//get_region_superdisk_diamond();
	//get_region_superdisk_concave();
  ...
  //*********************************************************************
	...
	for(int q = 0; q < TN; q++)
    {
	  ...
	  //printf("%f   %d change has finished... \n", energyt, i+1 );
		...
		plot_Chi_k(); //fill the matrix in order to generate the full plot
