...
int Nk = 300; //not larger than MAXX/2, for stealthy can be chosen smaller
double Kspace = 2 * pi / MAXX;
double Kbin = 0.036;
#define Kbin_num 150
double J_k_Re[601][601]; //2Nk+1, make sure consistency
double J_k_Im[601][601];
double dJ_k_Re[601][601];
double dJ_k_Im[601][601];
...
double energy(double Chi_k[Kbin_num])
{
  ...
  for(int i = 0; i < Kbin_num; i++)
    {
      E = E + (Chi_k[i] - obj[i]) * (Chi_k[i] - obj[i]);
    }
...
double d_energy(double Chi_k[Kbin_num], double Chi_k_T[Kbin_num])
{
  double d_E = 0;
  d_E = energy(Chi_k_T) - energy(Chi_k);
  return d_E;
}
  ...
  //*********************************************************************
	...
	for(int q = 0; q < TN; q++)
    {
	  ...
	  //Now we use Monte Carlo Steps
		...
		if( P > PE(d_energy(Chi_k, Chi_k_T), T))
	    {
			resume_config();
	  ...
	  //printf("%f   %d change has finished... \n", energyt, i+1 );
		...
		fp = fopen("Chi_k_b.txt", "w");
		for(int r = 0; r < Kbin_num; r++)
		{
			fprintf(fp, "%1.15f \t %1.15f\n", K_Pos[r], Chi_k_b[r]);
		}
		fclose(fp);

		fp = fopen("Chi_k.txt", "w");
		for(int r = 0; r < Kbin_num; r++)
		{
			fprintf(fp, "%1.15f \t %1.15f\n", K_Pos[r], Chi_k[r]);
		}
